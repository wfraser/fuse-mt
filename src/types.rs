...
pub trait FilesystemMT {
    ...
    async fn getattr(&self, _req: RequestInfo, _path: &Path, _fh: Option<u64>) -> ResultEntry {
        Err(libc::ENOSYS)
    }
    ...
    async fn readlink(&self, _req: RequestInfo, _path: &Path) -> ResultData {
        Err(libc::ENOSYS)
    }
    ...
    async fn unlink(&self, _req: RequestInfo, _parent: &Path, _name: &OsStr) -> ResultEmpty {
        Err(libc::ENOSYS)
    }
    ...
    async fn rmdir(&self, _req: RequestInfo, _parent: &Path, _name: &OsStr) -> ResultEmpty {
        Err(libc::ENOSYS)
    }
    ...
    async fn open(&self, _req: RequestInfo, _path: &Path, _flags: u32) -> ResultOpen {
        Err(libc::ENOSYS)
    }
    ...
    // NOTE: this is written out as a slightly modified version of what async-trait would emit if
    // this was written `async fn read`. Once https://github.com/dtolnay/async-trait/issues/126 is
    // fixed, this can be written the normal way.
    fn read<'life0, 'life1, 'async_trait>(
        ...
        offset: u64,
        size: u32,
        callback: impl for<'r> FnOnce(ResultSlice<'r>) -> CallbackResult + Send + 'async_trait,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = CallbackResult> + Send + 'async_trait>>
        where 'life0: 'async_trait,
              'life1: 'async_trait,
              Self: Sync + 'async_trait,
    {
        async fn __default<'async_trait, AsyncTrait: ?Sized + FilesystemMT + Sync>(
            _self: &AsyncTrait,
            _req: RequestInfo,
            _path: &Path,
            _fh: u64,
        ...
        Box::pin(__default(self, req, path, fh, offset, size, callback))
    }
    ...
    async fn opendir(&self, _req: RequestInfo, _path: &Path, _flags: u32) -> ResultOpen {
        Err(libc::ENOSYS)
    }
    ...
    async fn readdir(&self, _req: RequestInfo, _path: &Path, _fh: u64) -> ResultReaddir {
        Err(libc::ENOSYS)
    }
    ...
    async fn statfs(&self, _req: RequestInfo, _path: &Path) -> ResultStatfs {
        Err(libc::ENOSYS)
    }
    ...
    #[cfg(target_os = "macos")]
    async fn setvolname(&self, _req: RequestInfo, _name: &OsStr) -> ResultEmpty {
        Err(libc::ENOSYS)
    }
    ...
    #[cfg(target_os = "macos")]
    async fn getxtimes(&self, _req: RequestInfo, _path: &Path) -> ResultXTimes {
        Err(libc::ENOSYS)
    }
}
